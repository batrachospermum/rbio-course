...
wrapper <- function(train.data, train.response,
  ...
  rmse_training <- lapply(rf, function(x) sqrt((1/40)*(sum(predict(x, train.data) - train.response)) ** 2)) 
  rmse_val <- lapply(rf, function(x) sqrt((1/10)*(sum(predict(x, test.data) - test.response)) ** 2))
...
Переобучения нет, было выбрано значение 8.
...
В результате получилось, что подобранные параметры выдают ошибку чуть большую для тренирующей выборки, но намного меньшую для выборки валидирующей.